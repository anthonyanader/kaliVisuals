...
exports.getTwitterData = functions.database.ref('monitoredTags/{tagId}').onCreate((snapshot, context) => {
    ...
    twitterDataPromise.then(sentimentScore => {
        const sentimentObject = {
            sentimentScore: sentimentScore,
            date: moment().format()
        }

        database.ref('/monitoredTags')
                .child(tagId)
                .child('sentimentBucket').push().set(sentimentObject);
        return;
    })
    .catch(err => {
        console.log(err)
...
exports.scheduledSentimentBatch = functions.https.onRequest((req, res) => {
    database.ref('/monitoredTags').once('value', (data) => {
